...
//echo "-PageflipCategory-2-".$categoryid."<P>";

			$this->getCategory(intval($categoryid));
//echo "-PageflipCategory-3-".$categoryid."<P>";
		} else {
//echo "-PageflipCategory-4-".$categoryid."<P>";
...
//echo "BROCH1:".$this->brochure_id;
	...
	function getChildTreeArray($sel_id=0, $order='category_id', $perms='',$parray = array(), $r_prefix='')
	{

		$sql = "SELECT * FROM ".$this->table." WHERE (category_pid=".$sel_id.")".$perms;
		if ( $order != "" ) {
...
//echo "KEY:".$key." VAL:".$value."<P>";
	...
	function store()
	{
		...
		$category_description =$myts->addSlashes($this->category_description);
		...
		if ( empty($this->category_id) ) {
			...
			//$sql = sprintf("INSERT INTO %s (category_id, category_pid, category_title, category_description, category_imgurl) VALUES (%u, %u, '%s', '%s', '%s')", 'can_pageflip_categories', 0, 0, 'sdf', 'sdfsdf','');
...
//echo "STORE-2<P>";
			...
			$sql = sprintf("UPDATE %s SET category_pid = %u, category_title = '%s', category_description='%s', category_imgurl = '%s' WHERE category_id = %u", $this->table, intval($this->category_pid), $title, $category_description, $imgurl, intval($this->category_id));
		}
	...
	function &getCategoriesList($frontpage=false,$perms=false)
	{
		...
		while ($myrow = $this->db->fetchArray($result)) {
			$ret[$myrow['category_id']] = array('cat_title' => $myts->displayTarea($myrow['category_title']), 'cat_id' => $myrow['category_id'], 'cat_pid' => $myrow['category_pid']);
		}
		return $ret;
	}
...
//echo "deleteSingleCategory-1<P>SQL:".$sql."<P>";
